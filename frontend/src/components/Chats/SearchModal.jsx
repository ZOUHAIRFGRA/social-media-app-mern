...
const NewDialog = ({ open, onClose }) => {
    ...
    return (
        ...
        <div className="flex flex-col w-80 sm:w-96 bg-gray-900 text-white">
            <div className="flex justify-between items-center px-4 border-b py-2.5">
                <span className="font-medium mx-auto">New Message</span>
                <svg onClick={onClose} className="cursor-pointer" aria-label="Close" color="#ffffff" fill="#ffffff" height="24" role="img" viewBox="0 0 24 24" width="24">
            ...
            <div className="flex flex-col overflow-x-hidden h-96 w-full">
                {loading ?
                    ...
                    : users.length > 0 ?
                        users.map((u) => (
                            <div onClick={() => addToChat(u._id)} className="flex items-center hover:bg-gray-800 py-2 px-4 cursor-pointer" key={u._id}>
