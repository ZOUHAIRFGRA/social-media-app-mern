...
const SearchBox = () => {
    ...
    return (
        <ClickAwayListener onClickAway={handleClickAway}>
            <div className="hidden sm:flex items-center gap-3 pl-4 ml-36 w-64 py-2 bg-gray-800 text-white rounded-lg relative">
                {!searching && <SearchIcon />}
                <input
                    className="bg-transparent text-sm border-none outline-none flex-1 pr-3 text-white"
                    type="search"
                    value={searchTerm}
                    onFocus={handleFocus}
                ...
                {searchResult &&
                    <>
                        <div className="absolute right-1/2 -bottom-5 rotate-45 h-4 w-4 drop-shadow-lg bg-gray-800 rounded-sm border-l border-t border-gray-700"></div>

                        <div className={`${loading ? 'justify-center items-center' : users.length < 1 && 'justify-center items-center'} absolute overflow-y-auto overflow-x-hidden flex flex-col top-[49px] w-[23rem] -left-11 h-80 bg-gray-800 text-white drop-shadow-xl rounded`}>
